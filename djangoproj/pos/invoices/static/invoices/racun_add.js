...
function lookup_stranka(sifra_stranke) {
    $.get('/invoice/stranka_details/', {'q':sifra_stranke}, function(data) {
...
function onblur_Artikel(event) {
    ...
    var row = RegExp.$1;
    if (sifra_artikla === '') {
        selector_prefix = '#' + 'id_racunpozicija_set-' +  row + '-'; 
...
function lookup_artikel(sifra_artikla, row) {
    selector_prefix = '#' + 'id_racunpozicija_set-' +  row + '-'; 
    $.get('/invoice/artikel_details/', {'q':sifra_artikla}, function(data) {
...
function insert_resultTemplate(ev) {
    ...
    $.get('/invoice/search/' + table_name + '/', {"q": q}, function(data) {
